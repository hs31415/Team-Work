...
public class SpawnAndMove : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0)) // 当鼠标左键点击时
        {
            ...
            objCost ObjCost = prefab.GetComponent<objCost>(); // 获取预设体上的ObjCost组件
            Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition); // 从鼠标点击位置发射一条射线
            RaycastHit hitInfo;

            if (Physics.Raycast(ray, out hitInfo))
            {
                if (hitInfo.collider.gameObject == this.gameObject && Cost >= ObjCost.cost) // 检查点击的对象是否是当前挂载该脚本的对象，以及当前费用是否足够
                {
                    ...
                    CostSystem costSystem = textObject.GetComponent<CostSystem>();
                    costSystem.currentCost -= ObjCost.cost;
